...
int main(void)
{
    /* ====== 诊断0: 原始寄存器写 LED (不依赖 HAL/SysTick) ====== */
    /* 使能 GPIOC 时钟 */
    RCC->APB2ENR |= RCC_APB2ENR_IOPCEN;
    /* PC13 推挽输出 */
    GPIOC->CRH &= ~0x00F00000;
    GPIOC->CRH |= 0x00300000;   /* 50MHz push-pull */

    /* 闪 3 次确认进入 main */
    for (int i = 0; i < 3; i++)
    {
        GPIOC->BSRR = GPIO_BSRR_BR13;  /* 亮 (低电平) */
        for (volatile uint32_t d = 0; d < 800000; d++) { }
        GPIOC->BSRR = GPIO_BSRR_BS13;  /* 灭 */
        for (volatile uint32_t d = 0; d < 800000; d++) { }
    }
    ...
    /* 时钟配置 (HSE 8MHz → PLL ×9 → 72MHz) */
    /* 如果 HSE 晶振不起振, 这里会超时死循环 */
    SystemClock_Config();

    /* GPIO 初始化 (会覆盖上面的寄存器设置, 但 LED 仍可用) */
    GPIO_Init();
